...
import { getVersionInfo } from '../middleware/versioning';
...
router.get('/openapi.json', (req: Request, res: Response) => {
  try {
    const specPath = join(__dirname, '../openapi/spec.yaml');
...
<body>
  ...
  <div class="version-info">
    ...
    <p><strong>Current Version:</strong> ${getVersionInfo().currentVersion}</p>
    <p><strong>Supported Versions:</strong> ${getVersionInfo().supportedVersions.join(', ')}</p>
    <p><strong>Deprecated Versions:</strong> ${getVersionInfo().deprecatedVersions.join(', ') || 'None'}</p>
  </div>
...
router.get('/status', async (req: Request, res: Response) => {
  try {
    ...
    const stats = await analyticsService.getUsageStats('day');
