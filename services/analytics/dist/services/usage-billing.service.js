...
class UsageBillingService {
    ...
    async getUsageAnalytics(query) {
        try {
            ...
            const topWorkflows = aggs.top_workflows.buckets.map((bucket) => {
            ...
            // Process top users
            const topUsers = aggs.top_users.buckets.map((bucket) => {
            ...
            const executionsByEngine = {};
            aggs.executions_by_engine.buckets.forEach((bucket) => {
                executionsByEngine[bucket.key] = bucket.doc_count;
            });
            // Process executions by status
            const executionsByStatus = {};
            aggs.executions_by_status.buckets.forEach((bucket) => {
                executionsByStatus[bucket.key] = bucket.doc_count;
            });
            // Process time series
            const timeSeries = aggs.executions_over_time.buckets.map((bucket) => ({
    ...
    calculateExecutionCosts(usageData) {
        let totalCost = 0;
        const byEngine = {};
        ...
        const byDuration = usageData.timeSeries.map(point => ({
    ...
    async calculateResourceCosts(organizationId, timeRange) {
        try {
    ...
    async calculateCostsByUser(organizationId, timeRange, usageData) {
        const costByUser = {};
    ...
    async calculateCostTrends(organizationId, timeRange, currentCost) {
        try {
    ...
    calculateProjectedCost(currentCost, trends) {
    ...
    async getUsageSummary(organizationId, timeRange) {
        try {
            const usageQuery = {
                organizationId,
                timeRange
    ...
    async getUsageAlerts(organizationId) {
        const alerts = [];
        try {
            const timeRange = {
    ...
    getMostUsedEngine(executionsByEngine) {
        let maxEngine = '';
        let maxCount = 0;
    ...
    calculateOverallSuccessRate(executionsByStatus) {
    ...
    calculateOverallErrorRate(executionsByStatus) {
        const failed = executionsByStatus.failed || 0;
        const total = Object.values(executionsByStatus).reduce((sum, count) => sum + count, 0);
        return total > 0 ? (failed / total) * 100 : 0;
    }
    getTopCostDriver(costByService) {
        let maxService = '';
        let maxCost = 0;
    ...
    async updateBillingRates(rates) {
...
//# sourceMappingURL=usage-billing.service.js.map