...
exports.ElasticsearchPipelineService = void 0;
const elasticsearch_1 = require("@elastic/elasticsearch");
const analytics_interface_1 = require("../interfaces/analytics.interface");
class ElasticsearchPipelineService {
    ...
    async ingestBulkData(dataItems) {
        try {
            ...
            if (response.errors) {
                const errorItems = response.items?.filter(item => item.index?.error || item.create?.error || item.update?.error || item.delete?.error);
    ...
    async createDataView(name, indexPattern, timeField = 'timestamp') {
        try {
    ...
    async deleteOldData(indexPattern, olderThanDays) {
        try {
    ...
    async getIndexStats(indexPattern) {
        try {
    ...
    getIndexName(dataType) {
        const typeMapping = {
            [analytics_interface_1.AnalyticsDataType.EXECUTION]: 'executions',
            [analytics_interface_1.AnalyticsDataType.WORKFLOW]: 'workflows',
            [analytics_interface_1.AnalyticsDataType.USER_ACTION]: 'user-actions',
            [analytics_interface_1.AnalyticsDataType.SYSTEM_METRIC]: 'system-metrics',
            [analytics_interface_1.AnalyticsDataType.PERFORMANCE]: 'performance',
            [analytics_interface_1.AnalyticsDataType.BILLING]: 'billing'
        };
...
exports.ElasticsearchPipelineService = ElasticsearchPipelineService;
//# sourceMappingURL=elasticsearch-pipeline.service.js.map