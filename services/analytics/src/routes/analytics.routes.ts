...
export function createAnalyticsRoutes(analyticsService: AnalyticsService, logger: Logger): Router {
  const router = Router();
  ...
  const analyticsDataSchema = Joi.object({
    ...
    type: Joi.string().valid(...Object.values(AnalyticsDataType)).required(),
  ...
  router.get('/dashboards/:dashboardId', async (req, res) => {
    try {
      ...
      const timeRangeQuery = Joi.object({
        start: Joi.date().required(),
        end: Joi.date().min(Joi.ref('start')).required()
      });
  ...
  router.get('/usage/:organizationId', async (req, res) => {
    try {
      ...
      const querySchema = Joi.object({
        ...
        includeWorkflows: Joi.boolean().default(true)
      });
